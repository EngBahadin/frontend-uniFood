...
function FoodItem({ layout, item }: FoodItemProps) {
  ...
  return (
    <div
      ...
      className={`${layout === "grid" ? "" : "md:w-64 sm:w-[200px] w-48"} flex flex-col rounded-2xl overflow-hidden bg-pure-white  md:h-[275px] sm:h-64 h-[200px] hover:scale-95`}
    >
      <div
        className="grid w-full h-1/2  place-items-center bg-primary-lm cursor-pointer p-2"
      ...
      <div className="flex justify-between p-2 sm:gap-x-3 gap-x-2">
        <div className="flex flex-col sm:gap-y-2 gap-y-1 sm:w-48 w-40 ">
          <h3
            className="lg:text-text-1-medium md:text-text-2-medium sm:text-text-3-medium text-caption-1-medium cursor-pointer"
            onClick={() => handleProductDetail(item.id)}
          >
            {item.name}
          </h3>

          <p className="lg:text-text-1-medium md:text-text-2-medium text-text-3-medium text-primary-lm">
            {item.price !== null
              ? item.price === 10
                ? 10000
                : item.price
          ...
          <p className="md:text-text-3-regular sm:text-caption-1-regular text-caption-2-regular text-gray-100 flex ">
            <span>
              <BsStarFill className="sm:size-4 size-3 mr-2 text-warning-lm" />
            </span>
